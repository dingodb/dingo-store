...
DEFINE_uint32(vector_index_hnsw_simd_test_dimension, 512, "vector index hnsw simd test dimension. default 512");
// DEFINE_uint32(vector_index_hnsw_simd_test_data_base_size, 1000000,
DEFINE_uint32(vector_index_hnsw_simd_test_data_base_size, 1000,
              "vector_index_hnsw_simd_test_data_base_size. default 1000000");

DEFINE_bool(vector_index_hnsw_simd_test_enable_simd, false, "vector_index_hnsw_simd_test_enable_simd. default false");

namespace dingodb {

class VectorIndexHnswSimdTest : public testing::Test {
 protected:
  static void SetUpTestSuite() {
    vector_index_thread_pool = std::make_shared<ThreadPool>("vector_index", 1);
    dimension = FLAGS_vector_index_hnsw_simd_test_dimension;
    data_base_size = FLAGS_vector_index_hnsw_simd_test_data_base_size;
    if (FLAGS_vector_index_hnsw_simd_test_enable_simd) {
      enable_simd = FLAGS_vector_index_hnsw_simd_test_enable_simd;
      VectorIndex::SetSimdHookForHnswlib();
    }
    LOG(INFO) << fmt::format(
        "vector_index_hnsw_simd_test_dimension: {} vector_index_hnsw_simd_test_data_base_size: {} "
        "vector_index_hnsw_simd_test_enable_simd : {}",
        FLAGS_vector_index_hnsw_simd_test_dimension, FLAGS_vector_index_hnsw_simd_test_data_base_size,
        FLAGS_vector_index_hnsw_simd_test_enable_simd ? "true" : "false");
  }

  static void TearDownTestSuite() {
    vector_index_hnsw_l2.reset();
    vector_index_hnsw_ip.reset();
  }
  ...
  inline static std::shared_ptr<VectorIndex> vector_index_hnsw_l2;
  inline static std::shared_ptr<VectorIndex> vector_index_hnsw_ip;
  // inline static faiss::idx_t dimension = 512;
  // inline static int data_base_size = 1000000;
  inline static faiss::idx_t dimension = FLAGS_vector_index_hnsw_simd_test_dimension;
  inline static int data_base_size = FLAGS_vector_index_hnsw_simd_test_data_base_size;
  inline static bool enable_simd = FLAGS_vector_index_hnsw_simd_test_enable_simd;
...
TEST_F(VectorIndexHnswSimdTest, Create) {
  ...
  // valid param L2
  {
    ...
    vector_index_hnsw_l2 = VectorIndexFactory::NewHnsw(id, index_parameter, kEpoch, kRange, vector_index_thread_pool);
    EXPECT_NE(vector_index_hnsw_l2.get(), nullptr);
  }
  ...
  // valid param IP
  {
    ...
    index_parameter.mutable_hnsw_parameter()->set_max_elements(max_elements);
    index_parameter.mutable_hnsw_parameter()->set_nlinks(nlinks);
    vector_index_hnsw_ip = VectorIndexFactory::NewHnsw(id, index_parameter, kEpoch, kRange, vector_index_thread_pool);
    EXPECT_NE(vector_index_hnsw_ip.get(), nullptr);
  }
}

TEST_F(VectorIndexHnswSimdTest, Add) {
  butil::Status ok;
  ...
  // add all
  {
    ...
    ok = vector_index_hnsw_l2->Add(vector_with_ids);
    EXPECT_EQ(ok.error_code(), pb::error::Errno::OK);
    ok = vector_index_hnsw_ip->Add(vector_with_ids);
    EXPECT_EQ(ok.error_code(), pb::error::Errno::OK);
  }
}

TEST_F(VectorIndexHnswSimdTest, SearchPrint) {
  butil::Status ok;
  ...
  auto lambda_search_function = [&lambda_time_now_function, &lambda_time_diff_microseconds_function,
                                 &lambda_get_instruction_function](std::shared_ptr<VectorIndex> vector_index_hnsw,
  ...
  auto lambda_search_function_wrapper = [&lambda_search_function](std::shared_ptr<VectorIndex> vector_index_hnsw,
                                                                  const char *name) {
    {
      int n = 1;
      uint32_t k = 3;
      lambda_search_function(vector_index_hnsw, name, n, k);
    }
  };

  lambda_search_function_wrapper(vector_index_hnsw_l2, "hnsw l2");

  lambda_search_function_wrapper(vector_index_hnsw_ip, "hnsw ip");
}

TEST_F(VectorIndexHnswSimdTest, Search) {
  butil::Status ok;
