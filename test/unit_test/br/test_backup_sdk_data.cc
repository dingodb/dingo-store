...
class BrBackupSdkDataTest : public testing::Test {
 protected:
...
TEST_F(BrBackupSdkDataTest, Init) {
  ...
  backup_sdk_data =
      std::make_shared<br::BackupSdkData>(coor_interaction, store_interaction, index_interaction, document_interaction,
                                          backupts, backuptso_internal, storage, storage_internal);
}

TEST_F(BrBackupSdkDataTest, SetRegionMap) {
  ...
  backup_sdk_data->SetRegionMap(region_maps);
}

TEST_F(BrBackupSdkDataTest, Filter) {
  butil::Status status = backup_sdk_data->Filter();
  EXPECT_EQ(status.error_code(), dingodb::pb::error::OK);
}

TEST_F(BrBackupSdkDataTest, Run) {
  butil::Status status = backup_sdk_data->Run();
  EXPECT_EQ(status.error_code(), dingodb::pb::error::OK);
}

TEST_F(BrBackupSdkDataTest, Backup) {
  butil::Status status = backup_sdk_data->Backup();
  EXPECT_EQ(status.error_code(), dingodb::pb::error::OK);
}

TEST_F(BrBackupSdkDataTest, GetBackupMeta) {
  std::shared_ptr<std::vector<dingodb::pb::common::BackupMeta>> backup_meta;
  backup_meta = backup_sdk_data->GetBackupMeta();
