...
class BrRestoreRegionDataTest;

static void TestImportDataToRegion(const BrRestoreRegionDataTest& br_restore_region_data_test,
                                   br::ServerInteractionPtr interaction, const std::string& group_belongs_to_whom,
                                   const std::string& region_file_name, const std::string& cf_file_name);
class BrRestoreRegionDataTest : public testing::Test {
 protected:
  ...
  friend void TestImportDataToRegion(const BrRestoreRegionDataTest& br_restore_region_data_test,
                                     br::ServerInteractionPtr interaction, const std::string& group_belongs_to_whom,
                                     const std::string& region_file_name, const std::string& cf_file_name);
...
static void TestImportDataToRegion(const BrRestoreRegionDataTest& br_restore_region_data_test,
                                   br::ServerInteractionPtr interaction, const std::string& group_belongs_to_whom,
                                   const std::string& region_file_name, const std::string& cf_file_name) {
  const std::string region_file_path = br_restore_region_data_test.base_dir + region_file_name;
  ...
  std::map<int64_t, std::shared_ptr<dingodb::pb::common::Region>> id_region_map;

  for (const auto& [region_id, region_ptr] : kvs) {
    dingodb::pb::common::Region region;
    auto ret = region.ParseFromString(region_ptr);
    EXPECT_TRUE(ret);
    id_region_map[region.id()] = std::make_shared<dingodb::pb::common::Region>(region);
  }

  const std::string cf_file_path = br_restore_region_data_test.base_dir + cf_file_name;
  ...
  std::map<int64_t, std::shared_ptr<dingodb::pb::common::BackupDataFileValueSstMetaGroup>> id_group_map;
  ...
  for (const auto& [region_id, group_ptr] : kvs) {
    ...
    id_group_map[std::stoll(region_id)] = std::make_shared<dingodb::pb::common::BackupDataFileValueSstMetaGroup>(group);
  }

  // EXPECT_EQ(id_region_map.size(), id_group_map.size());

  for (const auto& [region_id, group] : id_group_map) {
    auto restore_region_data = std::make_shared<br::RestoreRegionData>(
        br_restore_region_data_test.coor_interaction, interaction, id_region_map[region_id],
        br_restore_region_data_test.restorets, br_restore_region_data_test.restoretso_internal,
        br_restore_region_data_test.storage, br_restore_region_data_test.storage_internal, group, cf_file_name,
        group_belongs_to_whom, br_restore_region_data_test.restore_region_timeout_s);
...
TEST_F(BrRestoreRegionDataTest, Init) {
...
TEST_F(BrRestoreRegionDataTest, TestDocumentCfSstMetaSdkData) {
  ...
  TestImportDataToRegion(*this, interaction, group_belongs_to_whom, region_file_name, cf_file_name);
}

TEST_F(BrRestoreRegionDataTest, TestDocumentCfSstMetaSqlData) {
  br::ServerInteractionPtr interaction = br::InteractionManager::GetInstance().GetDocumentInteraction();
...
TEST_F(BrRestoreRegionDataTest, TestIndexCfSstMetaSdkData) {
...
TEST_F(BrRestoreRegionDataTest, TestIndexCfSstMetaSqlData) {
  br::ServerInteractionPtr interaction = br::InteractionManager::GetInstance().GetIndexInteraction();
...
TEST_F(BrRestoreRegionDataTest, TestStoreCfSstMetaSdkData) {
...
TEST_F(BrRestoreRegionDataTest, TestStoreCfSstMetaSqlData) {
  br::ServerInteractionPtr interaction = br::InteractionManager::GetInstance().GetStoreInteraction();
...
TEST_F(BrRestoreRegionDataTest, TestStoreCfSstMetaSqlMeta) {
  br::ServerInteractionPtr interaction = br::InteractionManager::GetInstance().GetStoreInteraction();
