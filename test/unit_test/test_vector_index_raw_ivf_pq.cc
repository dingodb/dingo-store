  ...
  static void ReCreate() {
    ...
    // valid param IP
    {
      ...
      vector_index_raw_ivf_pq_ip =
          VectorIndexFactory::NewIvfPq(id, index_parameter, k_epoch, kRange, vector_index_thread_pool);
    }
...
ThreadPoolPtr VectorIndexRawIvfPqTest::vector_index_thread_pool = nullptr;
...
TEST_F(VectorIndexRawIvfPqTest, DeleteNoData) {
  ...
  // id not found
  {
    ...
    ids.push_back(id);
    ok = vector_index_raw_ivf_pq_l2->Delete(ids);
...
TEST_F(VectorIndexRawIvfPqTest, SearchNotTrain) {
  butil::Status ok;
...
TEST_F(VectorIndexRawIvfPqTest, AddNotTrain) {
  butil::Status ok;
