...
class MvccGcTest : public testing::Test {
 protected:
  static void SetUpTestSuite() {
    ...
    pb::common::ScalarSchemaItem *item = MvccGcTest::scalar_schema.add_fields();
    item->set_key("col1");
    item->set_field_type(::dingodb::pb::common::ScalarFieldType::FLOAT32);
    item->set_enable_speed_up(true);

    item = MvccGcTest::scalar_schema.add_fields();
    item->set_key("col2");
    item->set_field_type(::dingodb::pb::common::ScalarFieldType::BYTES);
    item->set_enable_speed_up(true);
  }
  ...
  static inline pb::common::ScalarSchema scalar_schema;
...
static void DoGcCore(bool gc_stop, int64_t safe_point_ts) {
  ...
  std::string region_start_key(MvccGcTest::prefix_start_key);
  std::string region_end_key(MvccGcTest::prefix_end_key);
  ...
  definition.set_part_id(MvccGcTest::partition_id);
  definition.set_tenant_id(0);
  definition.mutable_index_parameter()->mutable_vector_index_parameter()->mutable_scalar_schema()->CopyFrom(
      MvccGcTest::scalar_schema);
  definition.mutable_index_parameter()->set_index_type(::dingodb::pb::common::IndexType::INDEX_TYPE_VECTOR);
  ...
  ctx->SetWriter(MvccGcTest::engine->Writer());
#endif

  Server::GetInstance().GetStoreMetaManager()->GetStoreRegionMeta()->AddRegion(region_ptr);

  status = TxnEngineHelper::DoGc(MvccGcTest::engine, raft_engine, ctx, MvccGcTest::safe_point_ts, gc_safe_point,
                                 region_start_key, region_end_key);
  Server::GetInstance().GetStoreMetaManager()->GetStoreRegionMeta()->DeleteRegion(region_id);
}

static void DeleteRange() {
  auto writer = MvccGcTest::engine->Writer();
  // delete range
  {
    pb::common::Range range;
    range.set_start_key(mvcc::Codec::EncodeKey(MvccGcTest::prefix_start_key, Constant::kMaxVer));
    range.set_end_key(mvcc::Codec::EncodeKey(MvccGcTest::prefix_end_key, Constant::kMaxVer));

    writer->KvDeleteRange({Constant::kVectorDataCF, Constant::kVectorScalarCF, Constant::kVectorScalarKeySpeedUpCF,
...
static void PrepareData(const std::vector<std::string> &prefix_key_array, int start_index, int end_index, int count,
  ...
  // prepare data
  {
    ...
    // write column
    {
      MvccGcTest::prefix_start_key = prefix_key_array[start_index];
      MvccGcTest::prefix_end_key = prefix_key_array[end_index];
      ...
      for (size_t i = 0; i < end_index; i++) {
        ...
        if (i == set_tso_position) {
          ...
          MvccGcTest::safe_point_ts = MvccGcTest::Tso2Timestamp(tso);
        }
      }
...
static void PrepareDataFunction(int j, int64_t physical, int64_t logical, const std::string &prefix_key,
  ...
  tso.set_physical(physical);
  tso.set_logical(logical);
  int64_t start_ts = MvccGcTest::Tso2Timestamp(tso);
  tso.set_logical(++logical);
  int64_t commit_ts = MvccGcTest::Tso2Timestamp(tso);
  ...
  switch (value_flag) {
    case mvcc::ValueFlag::kPut: {
      ...
      kv_scalar.set_key(default_key);
      mvcc::Codec::PackageValue(mvcc::ValueFlag::kPut, scalar_value);
      kv_scalar.set_value(scalar_value);

      kv_table.set_key(default_key);
      mvcc::Codec::PackageValue(mvcc::ValueFlag::kPut, table_value);
      kv_table.set_value(table_value);

      for (const auto &filed : MvccGcTest::scalar_schema.fields()) {
        if (filed.enable_speed_up()) {
          pb::common::KeyValue kv;
          int64_t region_part_id = MvccGcTest::partition_id;
          int64_t vector_id = VectorCodec::UnPackageVectorId(prefix_key);
          int64_t ts = commit_ts;

          std::string encode_key_with_ts =
              VectorCodec::EncodeVectorKey(Constant::kExecutorRaw, region_part_id, vector_id, filed.key(), ts);
          pb::common::ScalarValue pb_scalar_value;
          kv.set_key(encode_key_with_ts);
          scalar_speedup_value = pb_scalar_value.SerializeAsString();
          mvcc::Codec::PackageValue(mvcc::ValueFlag::kPut, scalar_speedup_value);
          kv.set_value(scalar_speedup_value);
          kv_scalar_speedups.push_back(kv);
        }
      }
      break;
    }
    case mvcc::ValueFlag::kPutTTL: {
      ...
      kv_scalar.set_key(default_key);
      mvcc::Codec::PackageValue(mvcc::ValueFlag::kPutTTL, ttl, scalar_value);
      kv_scalar.set_value(scalar_value);

      kv_table.set_key(default_key);
      mvcc::Codec::PackageValue(mvcc::ValueFlag::kPutTTL, ttl, table_value);
      kv_table.set_value(table_value);
      ...
      for (const auto &filed : MvccGcTest::scalar_schema.fields()) {
        if (filed.enable_speed_up()) {
          ...
          kv.set_key(encode_key_with_ts);
          scalar_speedup_value = pb_scalar_value.SerializeAsString();
          mvcc::Codec::PackageValue(mvcc::ValueFlag::kPutTTL, ttl, scalar_speedup_value);
          kv.set_value(scalar_speedup_value);
          kv_scalar_speedups.push_back(kv);
        }
      }
    ...
    case mvcc::ValueFlag::kDelete: {
      ...
      kv_scalar.set_key(default_key);
      scalar_value.clear();
      mvcc::Codec::PackageValue(mvcc::ValueFlag::kDelete, scalar_value);
      kv_scalar.set_value(scalar_value);

      kv_table.set_key(default_key);
      table_value.clear();
      mvcc::Codec::PackageValue(mvcc::ValueFlag::kDelete, table_value);
      kv_table.set_value(table_value);
      ...
      for (const auto &filed : MvccGcTest::scalar_schema.fields()) {
        if (filed.enable_speed_up()) {
          ...
          pb::common::ScalarValue pb_scalar_value;
          kv.set_key(encode_key_with_ts);
          scalar_speedup_value.clear();
          mvcc::Codec::PackageValue(mvcc::ValueFlag::kDelete, scalar_speedup_value);
          kv.set_value(scalar_speedup_value);
          kv_scalar_speedups.push_back(kv);
        }
      }
  ...
  writer->KvPut(Constant::kVectorDataCF, kv_default);
  writer->KvPut(Constant::kVectorScalarCF, kv_scalar);
  writer->KvPut(Constant::kVectorTableCF, kv_table);
  writer->KvBatchPut(Constant::kVectorScalarKeySpeedUpCF, kv_scalar_speedups);
}

// safe_point_ts > any data &&  safe_point_ts < any data  first put
TEST_F(MvccGcTest, DoGcNormalGcSafePointAtMiddleFirstPutAndGcStop) {
  int64_t vector_id = 1;
  ...
  for (auto &prefix_key : prefix_key_array) {
    prefix_key = VectorCodec::PackageVectorKey(Constant::kExecutorRaw, MvccGcTest::partition_id, vector_id++);
  }
  ...
  DoGcCore(true, MvccGcTest::safe_point_ts);
#if defined(ENABLE_GC_MOCK)
  auto reader = MvccGcTest::engine->Reader();
  std::shared_ptr<Snapshot> snapshot = MvccGcTest::engine->GetSnapshot();
  int64_t count = 0;
  reader->KvCount(Constant::kVectorDataCF, snapshot,
                  mvcc::Codec::EncodeKey(MvccGcTest::prefix_start_key, Constant::kMaxVer),
                  mvcc::Codec::EncodeKey(MvccGcTest::prefix_end_key, Constant::kMaxVer), count);
  EXPECT_EQ((6) * FLAGS_gc_delete_batch_count, count);
  reader->KvCount(Constant::kVectorScalarCF, snapshot,
                  mvcc::Codec::EncodeKey(MvccGcTest::prefix_start_key, Constant::kMaxVer),
                  mvcc::Codec::EncodeKey(MvccGcTest::prefix_end_key, Constant::kMaxVer), count);
  EXPECT_EQ((6) * FLAGS_gc_delete_batch_count, count);
  reader->KvCount(Constant::kVectorTableCF, snapshot,
                  mvcc::Codec::EncodeKey(MvccGcTest::prefix_start_key, Constant::kMaxVer),
                  mvcc::Codec::EncodeKey(MvccGcTest::prefix_end_key, Constant::kMaxVer), count);
  EXPECT_EQ((6) * FLAGS_gc_delete_batch_count, count);
  reader->KvCount(Constant::kVectorScalarKeySpeedUpCF, snapshot,
                  mvcc::Codec::EncodeKey(MvccGcTest::prefix_start_key, Constant::kMaxVer),
                  mvcc::Codec::EncodeKey(MvccGcTest::prefix_end_key, Constant::kMaxVer), count);
  EXPECT_EQ((6) * FLAGS_gc_delete_batch_count * 2, count);
  DeleteRange();
  snapshot = MvccGcTest::engine->GetSnapshot();
  reader->KvCount(Constant::kVectorDataCF, snapshot,
                  mvcc::Codec::EncodeKey(MvccGcTest::prefix_start_key, Constant::kMaxVer),
                  mvcc::Codec::EncodeKey(MvccGcTest::prefix_end_key, Constant::kMaxVer), count);
  EXPECT_EQ(0, count);
...
TEST_F(MvccGcTest, DoGcNoData) {
  ...
  DoGcCore(false, MvccGcTest::safe_point_ts);
#if defined(ENABLE_GC_MOCK)
  auto reader = MvccGcTest::engine->Reader();
  std::shared_ptr<Snapshot> snapshot = MvccGcTest::engine->GetSnapshot();
  int64_t count = 0;
...
TEST_F(MvccGcTest, DoGcNormalGcSafePointAtMiddleFirstPutTtl) {
  int64_t vector_id = 1;
  std::vector<std::string> prefix_key_array = {
      std::string() + "r" + std::to_string(partition_id) + "1",
      std::string() + "r" + std::to_string(partition_id) + "2",
  ...
  reader->KvCount(Constant::kVectorScalarKeySpeedUpCF, snapshot,
                  mvcc::Codec::EncodeKey(MvccGcTest::prefix_start_key, Constant::kMaxVer),
                  mvcc::Codec::EncodeKey(MvccGcTest::prefix_end_key, Constant::kMaxVer), count);
  EXPECT_EQ(((2) * FLAGS_gc_delete_batch_count) * 2, count);
  DeleteRange();
  snapshot = MvccGcTest::engine->GetSnapshot();
  reader->KvCount(Constant::kVectorDataCF, snapshot,
                  mvcc::Codec::EncodeKey(MvccGcTest::prefix_start_key, Constant::kMaxVer),
...
// safe_point_ts > any data &&  safe_point_ts < any data  first put
TEST_F(MvccGcTest, DoGcNormalGcSafePointAtMiddleFirstPut) {
  int64_t vector_id = 1;
  std::vector<std::string> prefix_key_array = {
      std::string() + "r" + std::to_string(partition_id) + "1",
      std::string() + "r" + std::to_string(partition_id) + "2",
  ...
  reader->KvCount(Constant::kVectorScalarKeySpeedUpCF, snapshot,
                  mvcc::Codec::EncodeKey(MvccGcTest::prefix_start_key, Constant::kMaxVer),
                  mvcc::Codec::EncodeKey(MvccGcTest::prefix_end_key, Constant::kMaxVer), count);
  EXPECT_EQ(((2) * FLAGS_gc_delete_batch_count + 4) * 2, count);
  DeleteRange();
  snapshot = MvccGcTest::engine->GetSnapshot();
  reader->KvCount(Constant::kVectorDataCF, snapshot,
                  mvcc::Codec::EncodeKey(MvccGcTest::prefix_start_key, Constant::kMaxVer),
...
TEST_F(MvccGcTest, DoGcNormalGcSafePointLessThanAnyDataTimestampFirstPut) {
  int64_t vector_id = 1;
  std::vector<std::string> prefix_key_array = {
      std::string() + "r" + std::to_string(partition_id) + "1",
      std::string() + "r" + std::to_string(partition_id) + "2",
  ...
  reader->KvCount(Constant::kVectorScalarKeySpeedUpCF, snapshot,
                  mvcc::Codec::EncodeKey(MvccGcTest::prefix_start_key, Constant::kMaxVer),
                  mvcc::Codec::EncodeKey(MvccGcTest::prefix_end_key, Constant::kMaxVer), count);
  EXPECT_EQ((prefix_key_array.size() - 2) * FLAGS_gc_delete_batch_count * 2, count);
  DeleteRange();
  snapshot = MvccGcTest::engine->GetSnapshot();
  reader->KvCount(Constant::kVectorDataCF, snapshot,
                  mvcc::Codec::EncodeKey(MvccGcTest::prefix_start_key, Constant::kMaxVer),
...
// safe_point_ts > any data timestamp first put
TEST_F(MvccGcTest, DoGcNormalGcSafePointGreaterAnyDataTimestampFirstPut) {
  int64_t vector_id = 1;
  std::vector<std::string> prefix_key_array = {
      std::string() + "r" + std::to_string(partition_id) + "1",
      std::string() + "r" + std::to_string(partition_id) + "2",
  ...
  reader->KvCount(Constant::kVectorScalarKeySpeedUpCF, snapshot,
                  mvcc::Codec::EncodeKey(MvccGcTest::prefix_start_key, Constant::kMaxVer),
                  mvcc::Codec::EncodeKey(MvccGcTest::prefix_end_key, Constant::kMaxVer), count);
  EXPECT_EQ((prefix_key_array.size() - 1) * 2, count);
  DeleteRange();
  snapshot = MvccGcTest::engine->GetSnapshot();
  reader->KvCount(Constant::kVectorDataCF, snapshot,
                  mvcc::Codec::EncodeKey(MvccGcTest::prefix_start_key, Constant::kMaxVer),
...
TEST_F(MvccGcTest, DoGcNormalGcSafePointGreaterAnyDataTimestampFirstPutTtl) {
  int64_t vector_id = 1;
  std::vector<std::string> prefix_key_array = {
      std::string() + "r" + std::to_string(partition_id) + "1",
      std::string() + "r" + std::to_string(partition_id) + "2",
...
TEST_F(MvccGcTest, DoGcNormalGcSafePointGreaterAnyDataTimestampFirstDelete) {
  int64_t vector_id = 1;
  std::vector<std::string> prefix_key_array = {
      std::string() + "r" + std::to_string(partition_id) + "1",
      std::string() + "r" + std::to_string(partition_id) + "2",
