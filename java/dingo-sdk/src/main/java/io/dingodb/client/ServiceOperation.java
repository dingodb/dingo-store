...
public class ServiceOperation {
    ...
    private static Map<DingoCommonId, StoreServiceClient> storeServiceClientMap = new ConcurrentHashMap<>();
    ...
    public Result operation(String tableName, StoreOperationType type, ContextForClient clientParameters) {
        ...
        do {
            try {
                ...
                Map<DingoCommonId, ContextForStore> keys2Store = groupKeysByStore(routeTable, tableName, contextForStore);

                List<Future<ResultForStore>> futures = new ArrayList<>();

                RouteTable finalRouteTable = routeTable;
                for (Map.Entry<DingoCommonId, ContextForStore> entry : keys2Store.entrySet()) {
                    DingoCommonId regionId = entry.getKey();
                    StoreServiceClient serviceClient = getStore(routeTable, regionId);
                    futures.add(
                        Executors.submit("do-operation",
                            () -> storeOperation.doOperation(finalRouteTable.getTableId(), serviceClient, entry.getValue()))
                    );
                }
    ...
    private Map<DingoCommonId, ContextForStore> groupKeysByStore(
            RouteTable routeTable, String tableName, ContextForStore wholeContext) {
        Map<DingoCommonId, List<byte[]>> keyListByStore = new ConcurrentHashMap<>();
        ...
        for (int index = 0; index < wholeContext.getStartKeyInBytes().size(); index++) {
            ...
            RangeDistribution part = getPartByStartKey(routeTable, keyInBytes);
            List<byte[]> keyList = keyListByStore.computeIfAbsent(part.getId(), k -> new ArrayList<>());
            keyList.add(keyInBytes);
        }

        Map<DingoCommonId, ContextForStore> contextGroupByStore = new ConcurrentHashMap<>();
        for (Map.Entry<DingoCommonId, List<byte[]>> entry : keyListByStore.entrySet()) {
            DingoCommonId regionId = entry.getKey();
            ...
            ContextForStore subStoreContext = ContextForStore.builder()
                    ...
                    .regionId(regionId)
                    .build();
            contextGroupByStore.put(regionId, subStoreContext);
        }
    ...
    public synchronized RouteTable getAndRefreshRouteTable(final String tableName, boolean isRefresh) {
        ...
        if (routeTable == null) {
            ...
            DingoCommonId tableId = metaClient.getTableId(table.getName());
    ...
    private StoreServiceClient getStore(final RouteTable routeTable, DingoCommonId regionId) {
        return storeServiceClientMap.computeIfAbsent(regionId,
                client -> routeTable.getLeaderStoreService(connection.getMetaClient()));
    }

    private RangeDistribution getPartByStartKey(final RouteTable routeTable, byte[] keyInBytes) {
