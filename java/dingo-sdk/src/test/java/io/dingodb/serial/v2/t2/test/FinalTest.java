...
import io.dingodb.serial.v2.t2.schema.*;
...
public class FinalTest {
    ...
    public static void test() {
        ...
        for (List<KeyValue> ukvs : ukvss) {
            ...
            re.updateValueByRecord(ukvs, record, index);
        }
        long tag4 = System.currentTimeMillis();

        for (int i = 0; i < ukvss.size(); i++) {
            List<KeyValue> kvs = kvss.get(i);
            List<KeyValue> ukvs = ukvss.get(i);
            kvs.set(0, ukvs.get(0));
            kvs.set(1, ukvs.get(1));
        }
