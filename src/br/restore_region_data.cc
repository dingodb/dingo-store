...
butil::Status RestoreRegionData::Run() {
  switch (region_->region_type()) {
    ...
    default:
      std::string s =
          group_debug_info_ + " unknown region type : " + dingodb::pb::common::RegionType_Name(region_->region_type());
      DINGO_LOG(ERROR) << s;
...
butil::Status RestoreRegionData::GetRegionEpoch() {
  ...
  status = interaction_->SendRequest("CoordinatorService", "QueryRegion", request, response);
  if (!status.ok()) {
...
butil::Status RestoreRegionData::SendRegionRequest(const std::string& service_name, const std::string& api_name) {
  ...
  while (true) {
    ...
    request.mutable_storage_backend()->mutable_local()->set_path(storage_);
    ...
    if (status.ok()) {
      if (response.error().errcode() == dingodb::pb::error::OK) {
        ...
        std::string s = group_debug_info_ + " " + response.error().errmsg();
        DINGO_LOG(ERROR) << s;
        return butil::Status(response.error().errcode(), s);
      }
    } else {
      if (status.error_code() != dingodb::pb::error::EREGION_VERSION) {
        std::string s = group_debug_info_ + " " + status.error_cstr();
        DINGO_LOG(ERROR) << group_debug_info_ + " " + status.error_cstr();
