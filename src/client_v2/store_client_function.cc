...
void SendVectorSearch(int64_t region_id, uint32_t dimension, uint32_t topn, std::string vector_data, std::string key,
  ...
  *(request.mutable_context()) = RegionRouter::GetInstance().GenConext(region_id);
  ...
  if (vector_data.empty()) {
    ...
    std::vector<float> row = dingodb::Helper::StringToVector(vector_data);
  ...
  if (with_scalar_pre_filter) {
    ...
    if (!scalar_filter_key.empty()) {
      if (std::string::size_type idx = scalar_filter_key.find("key"); idx != std::string::npos) {
        lambda_cmd_set_key_value_function(*vector->mutable_scalar_data(), scalar_filter_key, scalar_filter_value);

      } else {
        dingodb::pb::common::ScalarValue scalar_value;
        scalar_value.set_field_type(dingodb::pb::common::ScalarFieldType::STRING);
        dingodb::pb::common::ScalarField* field = scalar_value.add_fields();
        field->set_string_data(scalar_filter_value);

        vector->mutable_scalar_data()->mutable_scalar_data()->insert({scalar_filter_key, scalar_value});

        DINGO_LOG(INFO) << "scalar_filter_key: " << scalar_filter_key
                        << " scalar_filter_value: " << scalar_filter_value;
      }
    }

    if (!scalar_filter_key2.empty()) {
      if (std::string::size_type idx = scalar_filter_key2.find("key"); idx != std::string::npos) {
        lambda_cmd_set_key_value_function(*vector->mutable_scalar_data(), scalar_filter_key2, scalar_filter_value2);
      } else {
        dingodb::pb::common::ScalarValue scalar_value;
        scalar_value.set_field_type(dingodb::pb::common::ScalarFieldType::STRING);
        dingodb::pb::common::ScalarField* field = scalar_value.add_fields();
        field->set_string_data(scalar_filter_value2);

        vector->mutable_scalar_data()->mutable_scalar_data()->insert({scalar_filter_key2, scalar_value});

        DINGO_LOG(INFO) << "scalar_filter_key2: " << scalar_filter_key2
                        << " scalar_filter_value2: " << scalar_filter_value2;
      }
    }
  }

  if (with_scalar_post_filter) {
    request.mutable_parameter()->set_vector_filter(::dingodb::pb::common::VectorFilter::SCALAR_FILTER);
    ...
    if (!scalar_filter_key.empty()) {
      ...
      DINGO_LOG(INFO) << "scalar_filter_key: " << scalar_filter_key << " scalar_filter_value: " << scalar_filter_value;
    }
  ...
  if (with_table_pre_filter) {
    request.mutable_parameter()->set_vector_filter(::dingodb::pb::common::VectorFilter::TABLE_FILTER);
    request.mutable_parameter()->set_vector_filter_type(::dingodb::pb::common::VectorFilterType::QUERY_PRE);
  }

  if (ef_search > 0) {
    DINGO_LOG(INFO) << "ef_search=" << ef_search;
    request.mutable_parameter()->mutable_hnsw()->set_efsearch(ef_search);
  }

  if (bruteforce) {
    DINGO_LOG(INFO) << "bruteforce=" << bruteforce;
    request.mutable_parameter()->set_use_brute_force(bruteforce);
  }

  if (print_vector_search_delay) {
  ...
  std::ofstream file;
  if (!csv_output.empty()) {
    file = std::ofstream(csv_output, std::ios::out | std::ios::app);
    if (!file.is_open()) {
  ...
  if (with_scalar_pre_filter || with_scalar_post_filter) {
    std::vector<int64_t> result_vt_ids;
    for (const auto& vector_with_distance_result : response.batch_results()) {
      for (const auto& vector_with_distance : vector_with_distance_result.vector_with_distances()) {
        auto id = vector_with_distance.vector_with_id().id();
...
void SendVectorSearchDebug(int64_t region_id, uint32_t dimension, int64_t start_vector_id, uint32_t topn,
  ...
  *(request.mutable_context()) = RegionRouter::GetInstance().GenConext(region_id);
  ...
  if (with_vector_ids) {
    ...
    for (int i = 0; i < vector_ids_count; i++) {
      std::random_device seed;
      std::ranlux48 engine(seed());
...
void SendVectorScanDump(int64_t region_id, int64_t start_id, int64_t end_id, int64_t limit, bool is_reverse,
  ...
  std::ofstream file(csv_output, std::ios::out);
...
void SendDocumentScanQuery(int64_t region_id, int64_t start_id, int64_t end_id, int64_t limit, bool is_reverse,
  ...
  *(request.mutable_context()) = RegionRouter::GetInstance().GenConext(region_id);
  ...
  request.set_without_scalar_data(without_scalar);
  if (!key.empty()) {
    auto* keys = request.mutable_selected_keys()->Add();
    keys->assign(key);
  }
...
void SendCalcDistance(std::string vector_data1, std::string vector_data2) {
  if (vector_data1.empty() || vector_data2.empty()) {
    DINGO_LOG(ERROR) << "vector_data1 or vector_data2 is empty";
    return;
  }

  std::vector<float> x_i = dingodb::Helper::StringToVector(vector_data1);
  std::vector<float> y_j = dingodb::Helper::StringToVector(vector_data2);
  ...
  DINGO_LOG(INFO) << "vector_data1: " << vector_data1;
  DINGO_LOG(INFO) << " vector_data2: " << vector_data2;
...
void SendSnapshot(int64_t region_id) {
  ...
  DINGO_LOG(INFO) << response.DebugString();
  if (response.has_error() && response.error().errcode() != dingodb::pb::error::Errno::OK) {
    DINGO_LOG(ERROR) << "region " << region_id << " do  snapshot failed, error: "
                     << dingodb::pb::error::Errno_descriptor()->FindValueByNumber(response.error().errcode())->name()
                     << " " << response.error().errmsg();
  } else {
    std::cout << "region " << region_id << " do snapshot success " << std::endl;
  }
}
...
void BatchPutTable(std::shared_ptr<Context> ctx) {
  ...
  for (int i = 0; i < ctx->req_num; ++i) {
    ...
    std::string key = client_v2::Helper::EncodeRegionRange(partition_id) + Helper::GenRandomString(32);
    std::string value = Helper::GenRandomString(256);
