...
butil::Status CoordinatorInteraction::SendRequest(const std::string& api_name, const Request& request,
  ...
  DINGO_LOG(INFO) << "send request to coordinator api " << api_name << " request: " << request.ShortDebugString();
  int retry_count = 0;
  do {
    ...
    DINGO_LOG(INFO) << "send request to coordinator api " << api_name << " response: " << response.ShortDebugString();
    if (cntl.Failed()) {
