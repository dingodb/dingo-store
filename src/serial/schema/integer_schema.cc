...
void DingoSchema<std::optional<int32_t>>::InternalEncodeKey(Buf*  buf, int32_t data) {
...
void DingoSchema<std::optional<int32_t>>::InternalEncodeValue(Buf*  buf, int32_t data) {
  uint32_t* i = (uint32_t*)&data;
  buf->Write(*i >> 24);
  buf->Write(*i >> 16);
  buf->Write(*i >> 8);
