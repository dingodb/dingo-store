...
butil::Status RaftSnapshot::GenSnapshotFileByCheckpoint(const std::string& checkpoint_path, store::RegionPtr region,
  ...
  auto status = checkpoint->Create(checkpoint_path, Helper::GetColumnFamilyNames(range.start_key()), tmp_sst_files);
  if (!status.ok()) {
  ...
  sst_files = FilterSstFile(tmp_sst_files, range);
...
butil::Status RaftSnapshot::HandleRaftSnapshotRegionMeta(braft::SnapshotReader* reader, store::RegionPtr region) {
  ...
  if (!raw_cf_names.empty()) {
    status = engine_->Writer()->KvDeleteRange(raw_cf_names, range);
    if (!status.ok()) {
  ...
  if (!txn_cf_names.empty()) {
    pb::common::Range encode_range = region->Range(true);
    status = engine_->Writer()->KvDeleteRange(txn_cf_names, encode_range);
    if (!status.ok()) {
...
bool RaftSnapshot::LoadSnapshot(braft::SnapshotReader* reader, store::RegionPtr region) {
  ...
  auto range = region->Range(false);
  auto cf_names = Helper::GetColumnFamilyNames(range.start_key());
  ...
  if (Helper::IsExistPath(current_path)) {
    ...
    auto status = engine_->MergeCheckpointFiles(reader->get_path(), range, cf_names, merge_sst_file_paths);
    if (!status.ok()) {
      // Clean temp file
...
void SaveSnapshotByDingo(store::RegionPtr region, std::shared_ptr<RawEngine> /*engine*/, int64_t /*term*/,
  ...
  auto cf_names = Helper::GetColumnFamilyNames(region->Range(false).start_key());
  for (const auto& cf_name : cf_names) {
