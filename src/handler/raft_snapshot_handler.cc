...
butil::Status RaftSnapshot::GenSnapshotFileByCheckpoint(const std::string& checkpoint_path, store::RegionPtr region,
  ...
  auto status =
      checkpoint->Create(checkpoint_path, Helper::GetColumnFamilyNames(encode_range.start_key()), tmp_sst_files);
  if (!status.ok()) {
  ...
  sst_files = FilterSstFile(tmp_sst_files, encode_range);
...
butil::Status RaftSnapshot::HandleRaftSnapshotRegionMeta(braft::SnapshotReader* reader, store::RegionPtr region) {
  ...
  if (!raw_cf_names.empty()) {
    status = engine_->Writer()->KvDeleteRange(raw_cf_names, encode_range);
    if (!status.ok()) {
