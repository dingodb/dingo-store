...
DEFINE_int64(rpc_elapse_time_threshold_ms, 10, "rpc elapse time ms threshold");
...
butil::Status TxnIterator::Init(TrackerPtr tracker) {
  ...
  uint64_t snapshot_elapsed = Helper::TimestampUs() - step_time;
  DINGO_LOG_IF(INFO, FLAGS_dingo_log_switch_txn_detail)
  ...
  uint64_t write_iter_elapsed = Helper::TimestampUs() - step_time;
  DINGO_LOG_IF(INFO, FLAGS_dingo_log_switch_txn_detail)
      << fmt::format("[txn]Init start_ts:{}, new_write_iter elapsed:{} us", start_ts_, write_iter_elapsed);
  if (tracker) {
    tracker->RecordElapsedTime("init:new_write_iter", write_iter_elapsed, 0);
  }
  ...
  DINGO_LOG_IF(INFO, FLAGS_dingo_log_switch_txn_detail)
      << fmt::format("[txn]Init start_ts:{}, new_lock_iter elapsed:{} us", start_ts_, lock_iter_elapsed);
  if (tracker) {
    tracker->RecordElapsedTime("init:new_lock_iter", lock_iter_elapsed, 0);
  }
...
butil::Status TxnIterator::InnerSeek(const std::string &key) {
  ...
  uint64_t write_seek_time = Helper::TimestampMs();
  ...
  uint64_t write_seek_elapsed = Helper::TimestampMs() - write_seek_time;

  if (lock_seek_elapsed > FLAGS_txn_iterator_elapse_time_threshold_ms ||
      write_seek_elapsed > FLAGS_txn_iterator_elapse_time_threshold_ms) {
    DINGO_LOG_IF(INFO, FLAGS_dingo_log_switch_txn_detail)
        << fmt::format("[txn]InnerSeek start_ts:{}, key:{}, lock_seek elapsed:{} ms, write_seek elapsed:{} ms",
                       start_ts_, Helper::StringToHex(key), lock_seek_elapsed, write_seek_elapsed);
  }
  ...
  uint64_t get_value_time = Helper::TimestampMs();
  auto ret = GetCurrentValue();
  uint64_t get_value_elapsed = Helper::TimestampMs() - get_value_time;
  if (ret.ok()) {
    if (get_value_elapsed > FLAGS_txn_iterator_elapse_time_threshold_ms) {
      DINGO_LOG_IF(INFO, FLAGS_dingo_log_switch_txn_detail)
          << fmt::format("[txn]InnerSeek GetCurrentValue start_ts:{}, key:{}, elapsed:{} ms", start_ts_,
                         Helper::StringToHex(key), get_value_elapsed);
    }
    DINGO_LOG_IF(INFO, FLAGS_dingo_log_switch_txn_detail)
...
butil::Status TxnIterator::GetUserValueInWriteIter(std::shared_ptr<Iterator> write_iter, RawEngine::ReaderPtr reader,
  ...
  while (write_iter->Valid()) {
    ...
    if (write_info.op() == pb::store::Op::Delete) {
      ...
      GotoNextUserKeyInWriteIter(write_iter, last_write_key, last_write_key, skipped_count);
      is_value_found = true;
      skipped_count += version_count;
      DINGO_LOG_IF(INFO, FLAGS_dingo_log_switch_txn_detail && version_count > 0) << fmt::format(
          "[txn]GetUserValueInWriteIter user_key:{}, start_ts:{}, scanned {} newer/rollback versions before "
          "Delete",
          Helper::StringToHex(user_key), start_ts, version_count);
      return butil::Status::OK();
...
butil::Status TxnEngineHelper::BatchGet(std::shared_ptr<Context> ctx, RawEnginePtr engine,
  ...
  for (const auto &key : keys) {
    ...
    uint64_t start_key_time = Helper::TimestampUs();
    ...
    uint64_t check_conflict_time = Helper::TimestampUs();
    if (check_conflict_time - start_key_time > FLAGS_txn_iterator_elapse_time_threshold_ms * 1000) {
      DINGO_LOG_IF(INFO, FLAGS_dingo_log_switch_txn_detail)
          << fmt::format("[txn]BatchGet start_ts:{}, key:{}, get lock and check lock conflict elapsed:{} us", start_ts,
                         Helper::StringToHex(key), check_conflict_time - start_key_time);
    }
    ...
    total_skip_versions += write_ver_count;
    uint64_t seek_time = Helper::TimestampUs();
    if (seek_time - check_conflict_time > FLAGS_txn_iterator_elapse_time_threshold_ms * 1000) {
      DINGO_LOG_IF(INFO, FLAGS_dingo_log_switch_txn_detail)
          << fmt::format("[txn]BatchGet start_ts:{}, key:{}, write_iter scan elapsed:{} us, version_count:{}", start_ts,
                         Helper::StringToHex(key), seek_time - check_conflict_time, write_ver_count);
    }
  ...
  if (Helper::TimestampUs() - start_time > FLAGS_rpc_elapse_time_threshold_ms * 1000) {
    DINGO_LOG(INFO) << "[txn]BatchGet keys_count: " << keys.size() << ", isolation_level: " << isolation_level
                    << ", start_ts: " << start_ts << ", elapsed:" << Helper::TimestampUs() - start_time << " us "
                    << ", total skip versions: " << total_skip_versions;
  }
  return butil::Status::OK();
}
...
butil::Status TxnEngineHelper::Scan(std::shared_ptr<Context> ctx, StreamPtr stream, RawEnginePtr raw_engine,
                                    const pb::store::IsolationLevel &isolation_level, int64_t start_ts,
  ...
  if (!current_stream_state) {
    ...
    if (Helper::TimestampUs() - seek_start_time > FLAGS_txn_iterator_elapse_time_threshold_ms * 1000) {
      DINGO_LOG_IF(INFO, FLAGS_dingo_log_switch_txn_detail) << fmt::format(
          "[txn]Scan start_ts:{}, iter->Seek elapsed:{} us.", start_ts, Helper::TimestampUs() - seek_start_time);
    }
