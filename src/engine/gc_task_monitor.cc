...
GcTaskMonitor& GcTaskMonitor::GetInstance() {
  static GcTaskMonitor instance;
  return instance;
}

void GcTaskMonitor::StartRun(int64_t start_time_ms) {
  ...
  if (first_start_time_ms_ == 0) {
    ...
    first_start_time_ms_bvar_.set_value(first_start_time_ms_);
  }
}

void GcTaskMonitor::AddRegionStats(int64_t region_id, int64_t start_time_ms, int64_t end_time_ms, int64_t iter_count,
                                   int64_t delete_count, int64_t safe_point_ts) {
  BAIDU_SCOPED_LOCK(mutex_);
  if (!current_run_.has_value()) {
    DINGO_LOG(ERROR) << fmt::format("[txn_gc] GcTaskMonitor AddRegionStats, has no value");
    return;
  }
...
void GcTaskMonitor::RemoveMissingRegionStats(const std::unordered_set<int64_t>& alive_region_ids) {
  BAIDU_SCOPED_LOCK(mutex_);
  if (alive_region_ids.empty()) {
    region_stats_.clear();
    return;
  }
...
void GcTaskMonitor::FinishRun(int64_t end_time_ms) {
  BAIDU_SCOPED_LOCK(mutex_);
  if (!current_run_.has_value()) {
    return;
  }
  ...
  ++run_count_;
  run_count_bvar_.set_value(run_count_);
  total_iter_count_ += current_run_->iter_count;
  total_delete_count_ += current_run_->delete_count;
  total_iter_count_bvar_.set_value(total_iter_count_);
  total_delete_count_bvar_.set_value(total_delete_count_);

  last_end_time_ms_ = end_time_ms;
  last_end_time_ms_bvar_.set_value(last_end_time_ms_);
...
void GcTaskMonitor::FillDebugInfo(pb::debug::DebugResponse::GCMetrics* gc_metrics,
  ...
  if (region_ids.empty()) {
    for (auto iter = history_.rbegin(); iter != history_.rend(); ++iter) {
      ...
      item->set_safe_point_ts(iter->safe_point_ts);
    }
  }
  ...
  if (region_ids.empty() && last_with_delete_.has_value()) {
    ...
    item->set_safe_point_ts(last_with_delete_->safe_point_ts);
  }
}

GcTaskMonitor::GcTaskMonitor()
    : first_start_time_ms_bvar_("dingo_txn_gc_first_start_time_ms", 0),
      run_count_bvar_("dingo_txn_gc_run_count", 0),
      last_end_time_ms_bvar_("dingo_txn_gc_latest_end_time_ms", 0),
      total_iter_count_bvar_("dingo_txn_gc_total_iter_count", 0),
      total_delete_count_bvar_("dingo_txn_gc_total_delete_count", 0) {}

ScopedGcTaskRun::ScopedGcTaskRun() : start_time_ms_(Helper::TimestampMs()) {
  GcTaskMonitor::GetInstance().StartRun(start_time_ms_);
}

ScopedGcTaskRun::~ScopedGcTaskRun() { GcTaskMonitor::GetInstance().FinishRun(Helper::TimestampMs()); }

void GetGcMetricsDebugInfo(pb::debug::DebugResponse::GCMetrics* gc_metrics, const std::vector<int64_t>& region_ids) {
  GcTaskMonitor::GetInstance().FillDebugInfo(gc_metrics, region_ids);
}
