...
butil::Status Writer::KvCompareAndSet(ColumnFamilyPtr column_family, const pb::common::KeyValue& kv,
  ...
  rocksdb::Status s = GetDB()->Get(rocksdb::ReadOptions(), column_family->GetHandle(),
  ...
  // write a key
  s = GetDB()->Put(rocksdb::WriteOptions(), column_family->GetHandle(),
                   rocksdb::Slice(kv.key().data(), kv.key().size()), rocksdb::Slice(value.data(), value.size()));
  if (!s.ok()) {
    DINGO_LOG(ERROR) << fmt::format("[rocksdb] put failed, error: {}.", s.ToString());
    return butil::Status(pb::error::EINTERNAL, "Internal put error");
  }
...
butil::Status Writer::KvPutIfAbsent(const std::string& cf_name, const pb::common::KeyValue& kv, bool& key_state) {
  ...
  return KvCompareAndSet(GetColumnFamily(cf_name), internal_kv, value, false, key_state);
}

butil::Status Writer::KvBatchPutIfAbsent(ColumnFamilyPtr column_family, const std::vector<pb::common::KeyValue>& kvs,
  ...
  for (const auto& kv : kvs) {
    ...
    s = batch.Put(column_family->GetHandle(), rocksdb::Slice(kv.key().data(), kv.key().size()),
...
butil::Status Writer::KvBatchPutIfAbsent(const std::string& cf_name, const std::vector<pb::common::KeyValue>& kvs,
                                         std::vector<bool>& key_states, bool is_atomic) {
  return KvBatchPutIfAbsent(GetColumnFamily(cf_name), kvs, key_states, is_atomic);
}
...
butil::Status Writer::KvCompareAndSet(const std::string& cf_name, const pb::common::KeyValue& kv,
  ...
  return KvCompareAndSet(GetColumnFamily(cf_name), kv, value, true, key_state);
}

butil::Status Writer::KvBatchCompareAndSet(ColumnFamilyPtr column_family, const std::vector<pb::common::KeyValue>& kvs,
  ...
  for (const auto& kv : kvs) {
    ...
    if (kv.value().empty()) {
      ...
      s = batch.Delete(column_family->GetHandle(), rocksdb::Slice(kv.key().data(), kv.key().size()));
      if (BAIDU_UNLIKELY(!s.ok())) {
        if (is_atomic) {
          key_states.clear();
          key_states.resize(kvs.size(), false);
...
butil::Status Writer::KvBatchCompareAndSet(const std::string& cf_name, const std::vector<pb::common::KeyValue>& kvs,
                                           const std::vector<std::string>& expect_values, std::vector<bool>& key_states,
                                           bool is_atomic) {
  return KvBatchCompareAndSet(GetColumnFamily(cf_name), kvs, expect_values, key_states, is_atomic);
}
...
butil::Status Writer::KvDeleteIfEqual(ColumnFamilyPtr column_family, const pb::common::KeyValue& kv) {
  if (BAIDU_UNLIKELY(kv.key().empty())) {
    DINGO_LOG(ERROR) << fmt::format("[rocksdb] not support empty key.");
    return butil::Status(pb::error::EKEY_EMPTY, "Key is empty");
  }
  ...
  // delete a key
  s = GetDB()->Delete(rocksdb::WriteOptions(), column_family->GetHandle(),
...
butil::Status Writer::KvDeleteIfEqual(const std::string& cf_name, const pb::common::KeyValue& kv) {
  return KvDeleteIfEqual(GetColumnFamily(cf_name), kv);
}
